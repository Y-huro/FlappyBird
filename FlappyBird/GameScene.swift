...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        ...
        // アイテム画像の大きさ調整
        //itemTexture.size().height = SKTexture(imageNamed: "").size().height
        ...
        // アイテムの上下の振れ幅を設定
        let random_y_range: CGFloat = 180
        // 空の中央位置(y座標)を取得
        let groundSize = SKTexture(imageNamed: "ground").size()
        let sky_center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        ...
        let createItemAnimation = SKAction.run({
            ...
            // アイテムを作成
            let item_obj = SKSpriteNode(texture: itemTexture)
            item_obj.position = CGPoint(x: 0, y: item_y)
            
            // スプライトに物理演算を設定する
            item_obj.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
            item_obj.physicsBody?.categoryBitMask = self.itemCategory
            // 衝突の時に動かないように設定する
            item_obj.physicsBody?.isDynamic = false
            item.addChild(item_obj)
